  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        if (clientId) {
          const clientDetails = await clientsApi.getById(clientId);
          ...
          setSelectedManagerId(managersData[0]._id);
        }
      } catch (err) {
                  ...
                  <div className="group flex flex-col gap-2">
                    ...
                    <div className={`relative flex items-center w-full h-12 bg-white rounded-md px-4 transition-all duration-200 ${
                      duplicateState.innMatch
                        ...
                        : "border border-[rgba(87,102,134,0.2)] hover:border-[#576686]/60 focus-within:border-[#2ABAEF] focus-within:ring-4 focus-within:ring-[#2ABAEF]/15"
                    }`}>
                      <input
                        type="text"
                        value={formData.inn}
