...
function EditHeaderCircleIcon() {
  return (
    <div className="size-[30px] bg-white rounded-full flex items-center justify-center hover:bg-slate-100 active:scale-95 transition-all duration-150 cursor-pointer shadow-xs">
      <svg className="w-3.5 h-3.5 text-[#576686]" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function NoteAddIcon({ className = "w-4 h-4" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function CreateTaskDocIcon({ className = "w-4 h-4" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path 
...
function TaskTypeCircleIcon({ type }: { type?: string }) {
  switch (type) {
    case "Встреча":
      return (
        <div className="size-11 bg-[#2ABAEF] rounded-full flex items-center justify-center text-white shadow-sm z-10">
          <Users className="w-5 h-5" />
        </div>
      );
    case "Отправить КП":
      return (
        <div className="size-11 bg-indigo-500 rounded-full flex items-center justify-center text-white shadow-sm z-10">
          <Send className="w-4 h-4" />
        </div>
      );
    case "Оплата":
      return (
        <div className="size-11 bg-amber-500 rounded-full flex items-center justify-center text-white shadow-sm z-10">
    ...
    default:
      return (
        <div className="size-11 bg-[#34D399] rounded-full flex items-center justify-center text-white shadow-sm z-10">
...
const formatFullForInput = (dateStr?: string | Date) => {
  if (!dateStr) return "12:00  /  20.08.2026";
  const d = new Date(dateStr);
  if (isNaN(d.getTime())) return "12:00  /  20.08.2026";
...
const parsePostponeDate = (dateStr: string): Date => {
  try {
    ...
    const timePart = parts.find((p) => p.includes(":")) || "12:00";
    const datePart = parts.find((p) => p.includes(".")) || "20.08.2026";
...
export default function ClientDetail() {
  ...
  const handleOpenPostponeModal = (task: any) => {
    ...
    setPostponeDate(formatFullForInput(dateToUse));
    setPostponeReason(task.postponeReason || "");
      ...
      <div className="mx-auto w-full max-w-[1500px]">
        ...
        <div className="relative w-full min-h-[82px] bg-[#576686] rounded-[10px] px-6 py-4 flex flex-wrap items-center justify-between gap-4 z-10 shadow-xs">
          ...
          <div className="flex items-center gap-3">
            ...
            <div onClick={() => navigate(`/clients/add?id=${client?._id || clientIdParam || ""}`)}>
          ...
          <div>
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => navigate(`/create-task?clientId=${client?._id || clientIdParam || ""}`)}
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                onClick={async () => {
                  ...
                  navigate(`/create-task?clientId=${client?._id || clientIdParam || ""}`);
                }}
